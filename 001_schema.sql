...
-- Migration: Add category to inventory_items if missing
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='inventory_items' AND column_name='category') THEN
        ALTER TABLE inventory_items ADD COLUMN category VARCHAR(50) NOT NULL DEFAULT 'Operational';
    END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS inventory_items (
    ...
    -- category: Generation | Infrastructure | Operational
...
VALUES
  ('INV-001','Distribution Transformers (11kV)','Infrastructure','units',50,10,142,2.5,100,
   '11111111-0000-0000-0000-000000000001','OK'),
  ('INV-002','Circuit Breakers (33kV)','Infrastructure','units',30,6,23,1.2,60,
   '11111111-0000-0000-0000-000000000002','Low'),
  ('INV-003','Power Cables (HT)','Infrastructure','meters',5000,1000,8500,120,10000,
   '11111111-0000-0000-0000-000000000003','OK'),
  ('INV-004','Smart Meters (AMI)','Operational','units',200,40,4,8.0,400,
   '11111111-0000-0000-0000-000000000001','Critical'),
  ('INV-005','Surge Arresters','Infrastructure','units',100,20,67,3.0,200,
   '11111111-0000-0000-0000-000000000002','Low'),
  ('INV-006','Insulators (Porcelain)','Infrastructure','units',500,100,312,15,1000,
   '11111111-0000-0000-0000-000000000004','Low'),
  ('INV-007','Relay Protection Units','Operational','units',40,8,31,1.0,80,
   '11111111-0000-0000-0000-000000000005','Low'),
  ('INV-008','Copper Conductors','Infrastructure','kg',1000,200,2,45.0,2000,
   '11111111-0000-0000-0000-000000000006','Critical'),
  ('INV-009','Solar Inverters (5kW)','Generation','units',20,4,15,0.5,30,
   '11111111-0000-0000-0000-000000000005','OK'),
  ('INV-010','Wind Turbine Blades','Generation','units',10,2,8,0.1,5,
   '11111111-0000-0000-0000-000000000002','OK')
