...
def create_order(req: CreateOrderRequest, db: Session = Depends(get_db)):
    ...
    # Send vendor email ONLY for manual orders, or stay in Pending Approval for auto triggers
    sent = False
    ...
    stage = "Vendor Notified" if sent else "Pending Approval"
    if req.trigger_type != "Manual":
        db.execute(
            text("UPDATE procurement_orders SET stage='Pending Approval' WHERE id=:id"),
            {"id": order_id}
        )
        db.commit()
    ...
    return {
        ...
        "stage":      "Vendor Notified" if sent else "Pending Approval",
        "email_sent": sent,
        "message":    f"Order {order_code} created. Status: {'Vendor Notified' if sent else 'Pending Approval'}.",
    }
...
def approve_order(order_id: str, db: Session = Depends(get_db)):
    order = _get_order(db, order_id)
    if order["stage"] != "Pending Approval":
        raise HTTPException(400, f"Order is in {order['stage']} stage, cannot approve")

    # Send vendor email now
    sent = send_vendor_order_email(
        ...
        confirm_token     = order["vendor_confirm_token"],
        total_price       = order["total_price"],
    )

    if not sent:
        raise HTTPException(500, "Failed to send vendor email")
    ...
    return {"message": f"Order {order['order_code']} approved and vendor notified"}


@router.post("/reject/{order_id}")
def reject_order(order_id: str, db: Session = Depends(get_db)):
    db.execute(
        text("UPDATE procurement_orders SET stage='Cancelled', updated_at=NOW() WHERE id=:id"),
        {"id": order_id}
    )
    db.commit()
    return {"message": "Order rejected and cancelled"}
