...
from notification_service import notify_stock_critical, notify_stock_low
...
@router.get("/overview")
def inventory_overview(category: Optional[str] = Query(None), db: Session = Depends(get_db)):
    sql = """
...
# ────────────────────────────────────────────────────────────────────────────
# COMPATIBILITY ALIASES (Prevent Frontend 404s)
# ────────────────────────────────────────────────────────────────────────────

@router.get("/orders/current")
def get_current_orders(db: Session = Depends(get_db)):
    """Return orders that are still in progress."""
    rows = db.execute(
        text("""
            SELECT o.*, i.name AS item_name, v.name AS vendor_name
...
def get_order_history(db: Session = Depends(get_db)):
    """Return orders that are completed or cancelled."""
    rows = db.execute(
        text("""
            SELECT o.*, i.name AS item_name, v.name AS vendor_name
            FROM procurement_orders o
            JOIN inventory_items i ON i.item_id = o.item_id
            JOIN vendors v ON v.id = o.vendor_id
            WHERE o.stage IN ('Delivered', 'Cancelled')
            ORDER BY o.updated_at DESC
        """)
    ).mappings().all()
    return [dict(r) for r in rows]
